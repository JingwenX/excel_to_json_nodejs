...
//json_file2 = 'C:\\Users\\xuh\\Desktop\\ori_excel_to_json\\excel_to_json_nodejs\\json_file\\upload_domain_name.json'
...
convertExcel(excel_file, json_file, options, function (err, data) {
                        if(err) {
                          ...
                          for (i in dict){
                            if ((dict[i]['CLASSIFICATION'].trim() != "") && (!dict[i]['CLASSIFICATION'].includes('Attribute heading') ) ){
                              //TODO: save the temp_dict to output dict 
                              //for(item in temp_dict) {
                              //  output[item] = temp_dict[item];
                              //}
                              if (data[i]['SUBCLASSIFICATION'].trim() != ""){
                              	key = dict[i]['CLASSIFICATION'].replace(reg, "").trim() + '/' + dict[i]['SUBCLASSIFICATION'].replace(reg, "").trim()
                              }
                              else{
                              	key = dict[i]['CLASSIFICATION'].replace(reg, "").trim()
                              }
                              temp_dict = {}
                              ...
                              for (attr in data[i]['ATTRIBUTE']){
                                ...
                                if (data[i]['ATTRIBUTE'][attr].trim() != ""){
                                  ...
                                  if (data[i]['SUBCLASSIFICATION'].trim() != ""){
                                    ...
                                    temp_dict[attr]['NAME'] = data[i]['ATTRIBUTE'][attr].replace(reg, "").trim()
